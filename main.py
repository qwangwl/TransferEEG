...
def load_seed(args, target, source_lists = None):
    ...
    # 获得源域数据
    source_features = torch.from_numpy(EEG[np.in1d(sGroup, source_lists)]).type(torch.Tensor)
    source_labels = torch.from_numpy(one_hot_mat[np.in1d(sGroup, source_lists)])
...
def train(target, args):
    ...
    np.savetxt(os.path.join(cur_target_saved_path, f"many.csv"), np_log, delimiter=",",  fmt='%.4f')
    return best_acc
